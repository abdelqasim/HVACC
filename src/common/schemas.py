"""
Pydantic schemas for HVAC FDD Platform (Pydantic v2)
"""
...
# Base model config:
# - Pydantic v2 reserves "model_*" internally, but your API uses fields like
#   model_loaded / model_version. Disable that protection to remove warnings.
class APIBaseModel(BaseModel):
    model_config = ConfigDict(
        protected_namespaces=(),  # allow model_loaded, model_version, etc.
        extra="ignore",           # ignore unexpected fields rather than failing
    )


class FaultTicket(APIBaseModel):
    """Fault ticket schema"""
...
class ExplanationResponse(APIBaseModel):
...
class DataValidationError(APIBaseModel):
